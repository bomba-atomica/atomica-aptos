...
#
# PURPOSE
# This script serves as the Docker entrypoint for validator nodes.
# It ensures that valid genesis artifacts exist before starting the `aptos-node` process.
#
# MODES OF OPERATION
# 1. Pre-generated Genesis (Standard Testnet Mode):
#    - Expects `genesis.blob` and `waypoint.txt` to be present in `/opt/aptos/var/genesis`.
#    - These are typically generated by `generate-genesis.sh` in the `DockerTestnet` setup phase
#      and mounted into the container.
#    - If found, it skips generation and immediately starts the node.
#
# 2. Inline Genesis (Fallback/Standalone Mode):
#    - If genesis artifacts are missing, each validator attempts to generate them deterministically.
#    - It uses environment variables ($VALIDATOR_INDEX, $NUM_VALIDATORS) to configure itself.
#    - This mode is useful for simple standalone tests but less flexible than the `DockerTestnet` orchestration.
#
# CONFIGURATION
# - $VALIDATOR_INDEX: The index of this validator (0 to N-1).
