...
Contrary to initial assumptions, the Timelock DKG is **not a sidecar**. It is a native component of the Aptos Validator binary (`aptos-node`), implemented primarily within the `dkg` crate. It reuses the robust DKG infrastructure built for Aptos Randomness but adapts it for Timelock specific needs (intervals vs epochs).
...
- **State**: Maintains separate channels for concurrent Timelock DKG sessions (since multiple intervals can be processing at once).
...
### Phase 1: Initiation
1.  **Trigger**: The `timelock.move` contract emits a `StartKeyGenEvent` (e.g., when a new interval is created).
...
3.  **Action**: `EpochManager::start_timelock_dkg` is called.
    ...
    - It spawns a new `DKGManager` task specifically for this interval.

### Phase 2: Generation (The "Audit" Phase)
1.  **Dealing**: The `DKGManager` generates a random secret scalar (Master Secret Key component) and computes the corresponding public key.
2.  **Sharing**: It effectively "splits" this secret using Shamir's Secret Sharing and encrypts a share for every other validator using their Consensus Public Key.
...
3.  **Decryption**: It uses its private Consensus Key to decrypt its specific share from the on-chain transcript.
...
1.  **Trigger**: The `timelock.move` contract emits a `RequestRevealEvent` (e.g., when the interval's time initializes).
...
```
*Note: In code (`epoch_manager.rs`), this is checked via `consensus_config.is_vtxn_enabled()`.*
...
While the native DKG *can* be enabled in the testnet by adding the `OnChainConsensusConfig::V5` (with `validator_txn` enabled) to the genesis layout, experiments have shown this to be unstable for end-to-end testing.
