...
async fn test_secret_revealed_after_deadline() {
    ...
    assert!(mpk_ready, "MPK must be published before testing secret revelation");
    info!("✅ MPK is ready");
    ...
    info!("Registering timelock {} with deadline {} (now={})", timelock_id, deadline, now);
    ...
    for attempt in 0..30 {
        match super::verify_secret_aggregated(&client, timelock_id, 1).await {
            Ok(secret) => {
                info!("✅ Secret revealed for timelock_id {} after {}s: {} bytes", 
                      timelock_id, attempt, secret.len());
                revealed = true;
                break;
            }
            Err(e) => {
