...
use crate::{
    network_interface::{BatchEncryptionDKGNetworkClient, DKGNetworkClient, RPC},
    types::{BatchEncryptionDKGMessage, DKGMessage},
};
use anyhow::bail;
...
impl RpcResponseSender for RealRpcResponseSender {
    fn send(&mut self, response: anyhow::Result<DKGMessage>) {
        ...
        let _ = self.inner.take().unwrap().send(rpc_response);
    }
}
...
pub struct BatchEncryptionDKGIncomingRpcRequest {
    pub msg: BatchEncryptionDKGMessage,
    pub sender: AccountAddress,
    pub response_sender: Box<dyn BatchEncryptionDKGResponseSender>,
}

#[derive(Clone)]
pub struct BatchEncryptionDKGNetworkSender {
    author: AccountAddress,
    dkg_network_client: BatchEncryptionDKGNetworkClient<NetworkClient<BatchEncryptionDKGMessage>>,
    self_sender: aptos_channels::Sender<Event<BatchEncryptionDKGMessage>>,
}

impl BatchEncryptionDKGNetworkSender {
    pub fn new(
        author: AccountAddress,
        dkg_network_client: BatchEncryptionDKGNetworkClient<
            NetworkClient<BatchEncryptionDKGMessage>,
        >,
        self_sender: aptos_channels::Sender<Event<BatchEncryptionDKGMessage>>,
    ) -> Self {
        BatchEncryptionDKGNetworkSender {
            author,
            dkg_network_client,
            self_sender,
        }
    ...
    pub async fn send_rpc(
        &self,
        receiver: AccountAddress,
        msg: BatchEncryptionDKGMessage,
        timeout_duration: Duration,
    ) -> anyhow::Result<BatchEncryptionDKGMessage> {
        if receiver == self.author() {
            let (tx, rx) = oneshot::channel();
            let protocol = RPC[0];
            let self_msg = Event::RpcRequest(self.author, msg.clone(), RPC[0], tx);
...
#[async_trait]
impl RBNetworkSender<BatchEncryptionDKGMessage> for BatchEncryptionDKGNetworkSender {
    async fn send_rb_rpc_raw(
        &self,
        receiver: AccountAddress,
        raw_message: Bytes,
...
pub struct BatchEncryptionDKGNetworkReceivers {
    pub rpc_rx: aptos_channel::Receiver<
        AccountAddress,
        (AccountAddress, BatchEncryptionDKGIncomingRpcRequest),
    >,
}

pub struct BatchEncryptionDKGNetworkTask {
    all_events: Box<dyn Stream<Item = Event<BatchEncryptionDKGMessage>> + Send + Unpin>,
...
impl BatchEncryptionDKGNetworkTask {
    pub fn new(
        network_service_events: NetworkServiceEvents<BatchEncryptionDKGMessage>,
        self_receiver: aptos_channels::Receiver<Event<BatchEncryptionDKGMessage>>,
    ) -> (
        BatchEncryptionDKGNetworkTask,
        BatchEncryptionDKGNetworkReceivers,
    ) {
        let (rpc_tx, rpc_rx) = aptos_channel::new(QueueStyle::FIFO, 10, None);
        ...
        if (network_and_events.values().len() != 1)
            || !network_and_events.contains_key(&NetworkId::Validator)
        {
            panic!("The network has not been setup correctly for Batch Encryption DKG!");
        }
        ...
        (
            BatchEncryptionDKGNetworkTask { rpc_tx, all_events },
            BatchEncryptionDKGNetworkReceivers { rpc_rx },
        )
    }
    ...
    pub async fn start(mut self) {
        while let Some(message) = self.all_events.next().await {
            match message {
                Event::RpcRequest(peer_id, msg, protocol, response_sender) => {
                    let req = BatchEncryptionDKGIncomingRpcRequest {
                        msg,
                        sender: peer_id,
                        response_sender: Box::new(BatchEncryptionDKGRealRpcResponseSender {
                            inner: Some(response_sender),
                            protocol,
                        }),
                    };
...
pub trait BatchEncryptionDKGResponseSender: Send + Sync {
    fn send(&mut self, response: anyhow::Result<BatchEncryptionDKGMessage>);
}

pub struct BatchEncryptionDKGRealRpcResponseSender {
    pub inner: Option<oneshot::Sender<Result<Bytes, RpcError>>>,
    pub protocol: ProtocolId,
}

impl BatchEncryptionDKGRealRpcResponseSender {
    pub fn new(raw_sender: oneshot::Sender<Result<Bytes, RpcError>>, protocol: ProtocolId) -> Self {
        Self {
            inner: Some(raw_sender),
            protocol,
...
impl BatchEncryptionDKGResponseSender for BatchEncryptionDKGRealRpcResponseSender {
    fn send(&mut self, response: anyhow::Result<BatchEncryptionDKGMessage>) {
        let rpc_response = response
            .and_then(|dkg_msg| self.protocol.to_bytes(&dkg_msg).map(Bytes::from))
            .map_err(RpcError::ApplicationError);
...
pub struct BatchEncryptionDKGDummyRpcResponseSender {
    pub rpc_response_collector: Arc<RwLock<Vec<anyhow::Result<BatchEncryptionDKGMessage>>>>,
}

impl BatchEncryptionDKGDummyRpcResponseSender {
    pub fn new(
        rpc_response_collector: Arc<RwLock<Vec<anyhow::Result<BatchEncryptionDKGMessage>>>>,
    ) -> Self {
        Self {
            rpc_response_collector,
        }
...
impl BatchEncryptionDKGResponseSender for BatchEncryptionDKGDummyRpcResponseSender {
    fn send(&mut self, response: anyhow::Result<BatchEncryptionDKGMessage>) {
        self.rpc_response_collector.write().push(response);
    }
}
