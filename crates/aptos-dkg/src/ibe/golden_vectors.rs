...
fn generate_golden_vectors() {
    ...
    writeln!(move_file, "/// IBE Golden Vector Fixtures (Test-Only)").unwrap();
    ...
    .unwrap();
    writeln!(move_file, "/// Auto-generated by Rust test generator.").unwrap();
    writeln!(
        move_file,
    ...
    for (i, v) in vectors.ibe_roundtrip_vectors.iter().enumerate() {
        ...
        .unwrap();
        for validator_shares in &v.dk_shares_g1_hex {
            writeln!(
                move_file,
            ...
            writeln!(move_file, "        vector::push_back(&mut v, inner);").unwrap();
        }
        writeln!(move_file, "        v").unwrap();
        writeln!(move_file, "    }}").unwrap();
        writeln!(move_file).unwrap();
        writeln!(
            move_file,
