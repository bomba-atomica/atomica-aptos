...
/// Serialize Gt element to bytes using uncompressed format.
/// This produces 576 bytes (12 Fq elements × 48 bytes each).
fn serialize_gt_to_bytes(gt: ark_bls12_381::Fq12) -> Result<Vec<u8>, SerializationError> {
    let mut bytes = Vec::new();
    gt.serialize_uncompressed(&mut bytes)?;
    Ok(bytes)
}
...
macro_rules! decrypt_internal_impl {
    (
        ...
        // Serialize K using uncompressed format (576 bytes)
        $context.charge($serialize_gas_cost)?;
        let k_bytes = serialize_gt_to_bytes(k_gt).map_err(|_e| abort_invariant_violated())?;
